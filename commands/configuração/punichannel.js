...
if(method != "info" && method != "delete" && method != "add") {
  ...
  .setDescription(`Use o comando : \`${prefix}punichannel <delete|add|info> <canal> !\``)
  .setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
  .setTimestamp();
 ...
 return  message.reply({embeds:[usagerr]}) 
    ...
    if (method == "delete") {
  let chx = db.get(`punichannel_${message.guild.id}`);
  
  if (chx == null) {
    const limite = new Discord.MessageEmbed()
.setColor('5fa5e3')

.setDescription(`<:fechar:918747498984665108> | Este servidor não tem nenhum canal de **" punichannel "** setado!`)
.setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
.setTimestamp();
...
.setDescription(`<:correto:918747498707824681> | O canal atual de **" punichannel "** é o <#${chx}> !`)
.setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
.setTimestamp();
