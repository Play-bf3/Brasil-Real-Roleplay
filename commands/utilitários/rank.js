...
let page = 1;
 let every = db.all().filter(data => data.ID.startsWith(`messages_${message.guild.id}_`)).sort((a, b) => b.data - a.data);
    let rank = every.map(m => m.ID).indexOf(`messages_${message.guild.id}_${message.author.id}`) + 1 || "N/A";
...
if(i1>=xp13.length){
 ...
 result = calc + i0
    ...
    if(xp[i].ID.startsWith(`messages_${message.guild.id}_`)) {
  ...
  let level = db.get(`level_${message.guild.id}_${user.id}`)   
   ...
   let curxp = xp[i].data
   if(curxp >= 1000 ) curxp = level/10 + 'K'
   let nxtLvlXp =  level * 100;
   let difference2 = nxtLvlXp  + 100 ;
   if( difference2 >= 1000 )difference2 = (level+1)  / 10 + 'K' ;
   if( level >= 1000 )level = level / 1000 + 'K' ;
   if(xp[i].data === 100000)difference2 =100 + 'K' ;
    content += `**<:forma:918749758632378408> ${i+1}º | ${user.tag}** \n **LEVEL:** \`${level}\`\n **XP:** \`${curxp}/${difference2}\`\n`
    ...
    .setTitle(`Sistema de Rank de Levels ${client.user.username}`)
