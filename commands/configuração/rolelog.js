...
let l1 = db.get(`logrole_${message.guild.id}`)
if(l1 == null) l1 = '\`Nenhum canal setado\`'
else l1 = `<#${l1}>`

    let msglog = db.get(`logrole_${message.guild.id}`)
    if(msglog == null) msglog = '\`Nenhum canal setado\`'
    else msglog = `<#${msglog}>`
    let msglogdel = db.get(`roledel_${message.guild.id}`)
    if(msglogdel == null) msglogdel = '<:Desligado:906795288797257778>'
    else msglogdel = '<:Ligado:906795236011933726>'
    let msglogedit= db.get(`rolecreate_${message.guild.id}`)
    ...
    .setTitle("Sistema de role log")    
    .setColor('5fa5e3')
    .setDescription(`<a:estrela:918882750742810654> Configure nosso canal de log abaixo!\n\n<:lunium:918729484365070376> Canal de log : ${msglog}\n<:lunidois:918729484407029870> Sistema de cargo criado : ${msglogedit}\n<:lunitres:918729484285411390>  Sistema de cargo deletado : ${msglogdel}\n<:luniquatro:918729484318965810> Salvar configuração`)    .setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
   .setTimestamp();
...
case 'butao2':
   ...
   if(msglogedit === '<:Desligado:906795288797257778>'){
    ...
    .setDescription(`<:e_fixadoTDN:844359619886579732> Hey, ${message.member} Você precisa setar o canal de role log `)
    .setColor('5fa5e3')
    .setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
    .setTimestamp();
  if(msglog == '\`Nenhum canal setado\`') return message.channel.send({embeds:[dadsad]}).then(msg=>  setTimeout(()=>{msg.delete()},4000))
    ...
    .setDescription(`<:correto:918747498707824681> | Sistema de cargo criado **habilitado.**`)
    .setColor('5fa5e3')
    ...
    .setDescription(`<:correto:918747498707824681> | Sistema de cargo criado **desabilitado.**`)
    .setColor('5fa5e3')
    ...
    .setTimestamp();
      ...
      case 'butao3':
        ...
        .setDescription(`<:correto:918747498707824681> | Sistema de cargo deletado **habilitado.**`)
        .setColor('5fa5e3')
        ...
        .setDescription(`<:correto:918747498707824681> | Sistema de cargo deletado **desabilitado.**`)
        .setColor('5fa5e3')
        ...
        case 'butao4':
          ...
          if(msglog !== l1){
            if(msglog == '\`Nenhum canal setado\`'){
              db.delete(`logrole_${message.guild.id}`)
            }else{
              db.set(`logrole_${message.guild.id}`,msglog.id)
              
            }
              }
            if(msglogdel == '<:Desligado:906795288797257778>'){
              db.delete(`roledel_${message.guild.id}`)
            }else{
              db.set(`roledel_${message.guild.id}`,'on')
              
            }
            if(msglogedit == '<:Desligado:906795288797257778>'){
              db.delete(`rolecreate_${message.guild.id}`)
            }else{
              db.set(`rolecreate_${message.guild.id}`,'on')
