...
let i1 = 10;
let page = 1;
 let every = db.all().filter(data => data.ID.startsWith(`invitotal_${message.guild.id}_`)).sort((a, b) => b.data - a.data);
    let rank = every.map(m => m.ID).indexOf(`invitotal_${message.guild.id}_${message.author.id}`) + 1 || "N/A";
    ...
    calc = 10-(i1 - xp13.length)
     result = calc + i0
        ...
        for (let i = 0; i < calc; i++) {
            ...
            if(xp[i].ID.startsWith(`invitotal_${message.guild.id}_`)) {
        ...
        .setTitle(`Sistema de Rank de Divulgações ${client.user.username}`)
