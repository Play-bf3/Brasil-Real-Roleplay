...
module.exports = {
  ...
  if(!message.guild.me.permissions.has("MANAGE_CHANNELS")) return  message.reply({embeds:[ad]})
    ...
    .setDescription(`<:fechado:918884825585319986> | O canal já esta** desbloqueado !**   `)
    .setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
    .setTimestamp();
    
    if(message.channel.rolePermissions(message.guild.roles.everyone).has('SEND_MESSAGES'))   return message.reply({embeds:[sndd]})
   ...
   SEND_MESSAGES: null
    ...
    .setColor('5fa5e3')
    .setTitle(`<:aberto:918884846959484948> Canal DESBLOQUEADO!    `)
    .setDescription(`O canal foi desbloqueado com sucesso por : ${message.author}    `)
    .setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
    .setTimestamp();
    return message.reply({embeds:[snd]});
